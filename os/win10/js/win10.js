...
window.Win10 = {
    ...
    _loadShortcut: function(){
        ...
        mzos.config.shortcuts.forEach(function(item){
    ...
    aboutUs: function() {
        //关于我们
         layer.open({
            ...
            title: mzos.name +' '+mzos.version,
            shadeClose: true, //开启遮罩关闭
            area: ['420px', '240px'], //宽高
            content: '<div style="padding: 10px;font-size: 12px">' +
            '<p>'+mzos.author.name+'版权所有</p>' +
            '<p>作者qq:'+mzos.author.qq+'</p>' +
            '<p>作者邮箱:'+mzos.author.email+'</p>' +
            '<p>作者博客:'+mzos.author.blog+'</p>' +
            '</div>'
        });
    },
